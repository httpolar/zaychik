...
class ReactRoleContextCommand : ContextualCommand<GuildMessageCommandInteractionCreateEvent> {
    ...
    override suspend fun action(event: GuildMessageCommandInteractionCreateEvent) {
        ...
        val providedRoleId = submission.interaction.textInputs
            .getOrDefault(roleIdFieldId, null)
            ?.value
            ?.trim()

        val providedEmoji = submission.interaction.textInputs
            .getOrDefault(emojiFieldId, null)
            ?.value
            ?.trim()
